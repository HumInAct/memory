...
function POSTagger(){
    this.lexicon = require('../../data/knowledge/lexicon');
}
...
POSTagger.prototype.tag = function(words){
    ...
    for (var i = 0; i < words.length; i++) {
        ...
        // rule: A word starting with a capital letter in the middle of the sentence is a NNP (except one letter and UH tag Hello..)
            ...
            else if (ret[i].match(/^NN/) && endsWith(words[i], "ses")) 
                ret[i] = "NNS";
            // rule: if a word has been categorized as a common noun and it ends with "s",
            //         then set its type to plural common noun (NNS)
            else if (ret[i] == "NN" && endsWith(words[i], "s") && /[^ous]$/.test(words[i].toLowerCase())) 
                ret[i] = "NNS";
            ...
            if (endsWith(words[i], "ly") && words[i].length > 3 && !words[i].match(/^(reply)$/))
                ret[i] = "RB";
            // rule: convert a common noun (NN or NNS) to an adjective if it ends with "al"
            if (ret[i].match(/^NN/) && word.match(/al$/)) 
                ret[i] = "JJ";

            // rule: convert a noun to a past participle if words[i] ends with "ed" and doesn't begin with a capital case
            if (ret[i].match(/^NN/) && endsWith(words[i], "ed") && /^[a-z]/.test(words[i])) 
                ret[i] = "VBN";
            ...
            // rule: convert a noun to a verb if the preceding work is a PRP with no verb before (except double verbs: made me do)
                ...
                && (!ret[i-2] || !ret[i-2].match(/^(VB|TO|IN)/) || words[i-2].match("^(that|"+doubleVerbs.join("|")+")$")) ) {
                if (ret[i] === "NNS")
                    ret[i] = "VBZ";
                else ret[i] = "VB";
            }
